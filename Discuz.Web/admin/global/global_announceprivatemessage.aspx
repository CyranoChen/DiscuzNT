<%@ Page Language="c#" Inherits="Discuz.Web.Admin.announceprivatemessage" Codebehind="announceprivatemessage.aspx.cs" %>
	...
	<cc1:DataGrid ID="DataGrid1" runat="server" IsFixConlumnControls="true" OnPageIndexChanged="DataGrid_PageIndexChanged" OnSortCommand="Sort_Grid">
		<Columns>
			<asp:TemplateColumn HeaderText="<input title='选中/取消' onclick='Check(this.form)' type='checkbox' name='chkall' id='chkall' />">
				...
				<ItemTemplate>
					<input id="id" onclick="checkedEnabledButton(this.form,'id','DelRec')" type="checkbox" value="<%# DataBinder.Eval(Container, "DataItem.pmid").ToString() %>"	name="id" />
					<%# DataGrid1.LoadSelectedCheckBox(DataBinder.Eval(Container, "DataItem.pmid").ToString())%>
				</ItemTemplate>
			</asp:TemplateColumn>
